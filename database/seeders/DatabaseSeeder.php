...
use App\Models\ { User,Contact, Address, Country, Product, Colissimo, Range, State, Shop, Page, Order };
...
;//
        ...
        ->each(function ($order) {
            $user = $order->user;
            $address = $user->addresses()->first()?->makeHidden(['id', 'user_id'])->toArray();
    
            if (!empty($address)) {
                $order->addresses()->create($address);
    
                // Add a second address conditionally
                if (mt_rand(0, 1)) {
                    $secondAddress = $user->addresses()->skip(1)->first()?->makeHidden(['id', 'user_id'])->toArray();
                    if (!empty($secondAddress)) {
                        $secondAddress['facturation'] = false;
                        $order->addresses()->create($secondAddress);
                    }
                }
            }
            ...
            $total = 0;
            $addProduct = function () use ($order, &$total) {
