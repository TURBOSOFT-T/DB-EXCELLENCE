            ...
            @scope('cell_color', $state)
               {{--  <x-badge value="   " class="px-6 py-2 bg-{{ $state->color }}-400" /> --}}
                  ...
                  {{--   <div style="display: flex; align-items: center; margin-bottom: 10px;">
                        <div style="width: 30px; height: 30px; background-color: {{ $state->color }}; border-radius: 5px; margin-right: 10px;"></div>
                        <span>{{ $state->name }}</span>
                    </div> --}}
                    
                 <div style="display: flex; align-items: center; margin-bottom: 10px;">
                    <span style="background-color: {{  $state->color }}; color: white; padding: 5px 10px; border-radius: 5px;">
                     {{ $state->name }}
                 </span>
             </div>
            @endscope
